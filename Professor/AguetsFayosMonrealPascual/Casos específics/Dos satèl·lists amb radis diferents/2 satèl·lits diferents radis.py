...
#CONSTANTS
m = [10,1,2]             #llista de masses normalitzades
R = [5,1,1]                       #llista de radis de les masses
c = 1.3                         #constant d'esmorteïment
w = 1                           #velocitat de l'aigua (normalitzada)

V = c/w                         #constant d'esmorteïment (normalitzada)
q0 = 1                          #alçada plana del mar (normalitzada)

k_q = 1000                     #coeficient de trasbalsament (moviment de marees d'un punt al següent)


#DISCRETITZACIÓ
T = 7500                        #nombre de Δt's
...
for i in range(Z):                                      #Posicions inicials del mar
    q[i][0] = q0 + 0.1*np.cos(4*np.pi*(i/Z)) + (1/30)*( np.exp(-pow(20*(i/Z),2))+np.exp(-pow(20*((i/Z)-1),2))-np.exp(-pow(20*((i/Z)-1/2),2)))
    q[i][1] = q0 + 0.1*np.cos(4*np.pi*(i/Z)) + (1/30)*( np.exp(-pow(20*(i/Z),2))+np.exp(-pow(20*((i/Z)-1),2))-np.exp(-pow(20*((i/Z)-1/2),2)))
...
marc = 1.5*np.max(r)           #marc per al plot d'òrbita
...
def update_plot(j):
    ...
    #ax.set(xlim=(r[0][j][0]-1.4*R[0],r[0][j][0]+1.4*R[0]), ylim=(r[0][j][1]-1.4*R[0],r[0][j][1]+1.4*R[0]))  #Seguiment proper de M
