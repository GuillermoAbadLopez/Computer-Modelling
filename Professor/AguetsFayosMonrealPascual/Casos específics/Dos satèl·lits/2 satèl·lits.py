...
#CONSTANTS
m = [5,1,1]                       #llista de masses
R = [5,1,1]                       #llista de radis de les masses
...
def update_plot(j):
    ...
    ax.set_xlabel(r'$x$')
    ax.set_ylabel(r'$y$')
    bx.set(xlim=(0,2*np.pi), ylim=(min(np.min(q)*0.9,0),max(np.max(q)*1.1,2*q0)))
    ...
    bx.plot(np.linspace(0, 2 * np.pi, Z), q[:, j], color='r', linestyle='-', label=r'$q(\theta,t)$')
    ...
    ax.add_patch(Polygon(np.asarray(Q),fill=False,edgecolor='r',linewidth=2,linestyle='-'))
...
step = 10               #interval entre cada temps j
