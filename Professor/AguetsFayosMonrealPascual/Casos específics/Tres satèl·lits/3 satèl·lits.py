...
#CONSTANTS
m = [5,1,1,1]                       #llista de masses
R = [5,1,1,1]                       #llista de radis de les masses
c = 1                           #constant d'esmorteïment
w = 1                           #velocitat de l'aigua

V = c/w                         #constant d'esmorteïment (normalitzada)
q0 = 1                          #alçada del mar (normalitzada)

k_q = 3000                      #coeficient de trasbalsament (moviment de marees d'un punt al següent)


#DISCRETITZACIÓ
T = 9000                        #nombre de Δt's
...
v[1][0][1] = 0
    ...
    #Massa 2
r[2][0][0] = -35*np.sqrt(3)/2
r[2][0][1] = -35*(1/2)
v[2][0][0] = 0.372*(1/2)
v[2][0][1] = -0.372*np.sqrt(3)/2


    #Massa 3
r[3][0][0] = 35*np.sqrt(3)/2
r[3][0][1] = -35*(1/2)
v[3][0][0] = 0.372*(1/2)
v[3][0][1] = 0.372*np.sqrt(3)/2
...
ax = fig.add_axes([1.25/30,1.25/10,8/30,8/10])          #N cossos
...
def update_plot(j):
    ...
    bx.text(0.1, 0.9, (r'$\gamma$ = %.2f' % (c)), size=10, color='black', ha='left', va='top', transform=bx.transAxes, position = (1/100,3/20))
    ax.set(xlim=(r[0][j][0]-50,r[0][j][0]+50), ylim=(r[0][j][1]-50,r[0][j][1]+50))
    #ax.set_title("Pla d'òrbita")
    ax.set_xlabel(r'$x$')
    ax.set_ylabel(r'$y$')
    bx.set(xlim=(0,2*np.pi), ylim=(0,2))
    ...
    bx.plot(np.linspace(0, 2 * np.pi, Z), q[:, j], color='lime', linestyle='-', label=r'$q(\theta,t)$')
    ...
    ax.add_patch(Polygon(np.asarray(Q),fill=False,edgecolor='lime',linewidth=2,linestyle='-'))
    ...
    if j == np.max(range(0,T,step)):
        fig.savefig('marees.png')


step = 50               #interval entre cada temps j
                                 # Atenció: Intervals petits i T's grans solen petar el programa
...
plt.clf()
plt.plot(np.linspace(0,2*np.pi,Z),q[:,-1])
plt.axhline(np.max(q[:,-1]),0,2*np.pi)
plt.savefig('alçades de marees.png')