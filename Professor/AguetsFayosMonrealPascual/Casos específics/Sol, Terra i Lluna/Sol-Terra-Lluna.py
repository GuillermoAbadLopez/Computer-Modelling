...
#CONSTANTS
m = [10,0.12,333000]             #llista de masses normalitzades
R = [5,1,500]                       #llista de radis de les masses
...
k_q = 10000                     #coeficient de trasbalsament (moviment de marees d'un punt al següent)
...
marc = 1.01*(np.max(R[0])+np.max(r))           #marc per al plot d'òrbita
...
def update_plot(j):
    ...
    ax.set(xlim=(r[0][j][0]-9*R[0],r[0][j][0]+9*R[0]), ylim=(r[0][j][1]-9*R[0],r[0][j][1]+9*R[0]))  #Seguiment proper de M
